...
cleanup(){
    docker container stop $(cat manager.id) $(cat worker-00.id) $(cat worker-01.id) $(cat personal.id) &&
        ...
        docker network rm $(cat advertise.id) $(cat listen.id) &&
        rm --force manager.id worker-00.id worker-01.id personal.id advertise.id listen.id
}
    trap cleanup EXIT &&
    docker network create $(uuidgen) --subnet 10.0.0.0/24 > advertise.id &&
    docker network create $(uuidgen) --subnet 10.0.1.0/24 > listen.id &&
    docker \
        container \
        create \
        --cidfile manager.id \
    ...
    docker network connect --alias manager --ip 10.0.1.200 $(cat listen.id) $(cat manager.id) &&
    docker network connect $(cat advertise.id) $(cat worker-00.id) &&
    docker network connect $(cat listen.id) $(cat worker-00.id) &&
    docker network connect $(cat advertise.id) $(cat worker-01.id) &&
    docker network connect $(cat listen.id) $(cat worker-01.id) &&
    ...
    docker container exec --interactive --tty $(cat manager.id) docker swarm init --advertise-addr 10.0.0.200 --listen-addr 10.0.1.200 &&
